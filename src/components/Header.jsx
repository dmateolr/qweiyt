...
const Header = () => {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full bg-stone-200 border-[1px] color-gray-100">
        <div className="p-3 max-w-4xl mx-auto   flex items-center justify-between ">
          ...
          <div>
            <div className="hidden sm:block p-2 rounded-lg bg-gray-100 items-center">
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex gap-2 text-gay-100">
                  <SearchIcon size={20} />
                  <input
                    type="text"
                    placeholder="Search"
          ...
          {user ? (
              ...
              <div className="flex items-center text-slate-800 text-2xl gap-3">
                ...
                {user ? (
                  ...
                  >
                    ...
                    />
                    {menuOpen && (
                      <div className="absolute z-30 -bottom-5 w-[200px] right-0 translate-y-full bg-#181A20 shadow rounded text-xs">
                        <ul className="flex flex-col p-3 justify-center gap-2">
                          <li onClick={() => setMenuOpen(!menuOpen)}>
                            <Link
                              to={`/${user?.username}`}
                              className="flex items-center text-white-100 gap-1"
                            >
                              <div>
                                <ProfileIcon />
                              </div>
            ...
            <div className="flex gap-3 items-center">
              <Link
                to="/login"
                className="bg-blue-500 text-white-100 font-semibold text-sm py-1 px-3 rounded"
              >
                Login
              </Link>
              <Link
      ...
      {modelOpen && (
        <div className="fixed top-0 flex items-center justify-center left-0 w-screen h-screen z-50">
          ...
          <button
            onClick={() => setModelOpen(false)}
            className="absolute md:top-7 md:right-10 top-5 right-3 text-white-100 md:text-5xl text-3xl"
          >
            <CloseIcon color="white" />
          </button>
          <div className="p-3 rounded-lg overflow-hidden">
            <div className="bg-stone-200 rounded-lg">
              <div className="border-gray-100 py-2 text-center text-lime-100">Create Post</div>
                    ...
                    <>
                      {images.length === 1 ? (
                        ...
                        <div className="relative">
                          <div className="flex gap-3 overflow-x-scroll snap-x text-white-100 ">
                  ...
                  <div className="flex justify-center w-full">
                    <div className="mb-3 w-full px-2">
                      {!images && (
                        <label
                          htmlFor="formFile"
                          className="text-2xl text-center block w-full mb-2 text-lime-100"
                        >
                      ...
                      <form onSubmit={uploadImage}>
                        {uploading ? (
                          <div className="rounded my-3 overflow-hidden w-full bg-blue-100">
                            <div
                              className="h-1 bg-blue-100 rounded transition-all"
                              style={{ width: `${percentage}%` }}
                            ></div>
                          </div>
                        ) : (
                          <input
                            className="block w-full px-3 mb-3 py-1.5 text-base font-normal text-white-100 bg-stone-200 bg-clip-padding border-gray-100 border-solid border-gray-100 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-#181A20 focus:border-blue-600 focus:outline-none"
                            type="file"
                            accept="image/*"
                            id="formFile"
                            // multiple
                        ...
                        {images && (
                          <>
                            <input
                              ...
                              className="p-2 border-gray-1 mb-3 outline-none w-full text-black"
                        ...
                        text-white-100 font-semibold text-sm rounded block text-center 
