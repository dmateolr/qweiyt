...
const ProfilePostCard = ({ post, span }) => {
  return (
    ...
    >
      <Link to={`/p/${post?.id}`}>
        <div className="relative group aspect-square overflow-hidden bg-gray-100  text-white-100  md:mb-1">
          <div className="absolute top-2 right-2 z-30 md:text-xl text-white-100">
            {post?.isVideo && <PlayIcon />}
            {post?.carouselMedia && <CollectionIcon className="rotate-180" />}
          </div>
          {/* overlay*/}
          <div className="absolute text-white-100 opacity-0 group-hover:opacity-100 transition-all cursor-pointer h-full w-full bg-black/40 z-10">
